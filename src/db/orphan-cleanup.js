...
// The audiobook schema reuses the `artists` table for book authors and
// series authors. Those references must ALSO keep an artist alive —
// otherwise a music scan's cleanup would try to delete an artist that's
// only an audiobook author, hitting the books/series FK (foreign_keys=ON)
// and aborting the scan, or (with ON DELETE SET NULL) silently stripping
// the book's authorship. The books/series probes prevent that. No
// table-exists guard is needed: cleanupOrphans only runs after the
// migration runner has brought the DB to the current SCHEMA_VERSION, so
// `books` and `series` always exist here.
const ORPHAN_ARTISTS_SQL = `SELECT id FROM artists
  WHERE NOT EXISTS (SELECT 1 FROM tracks            WHERE tracks.artist_id            = artists.id)
    ...
    AND NOT EXISTS (SELECT 1 FROM books             WHERE books.author_id             = artists.id)
    AND NOT EXISTS (SELECT 1 FROM series            WHERE series.author_id            = artists.id)
