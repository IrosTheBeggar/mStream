...
async function run() {
  ...
  // scans never run the cleanup pass. Runs before the audiobook dispatch
  // below — book libraries on network mounts face the same outage risk.
  ...
  if (loadJson.mediaType === 'audio-books') {
    try {
      // No expected-file pre-count: the old two-pass walk (and its
      // countSupportedFiles helper) was removed in favour of a
      // single-pass walk, so the progress row starts with a null
      // estimate and the bar runs indeterminate for audiobooks.
      try {
        progressStmts.insert.run(loadJson.scanId, loadJson.libraryId, loadJson.vpath || '', null);
      } catch (_) {}
