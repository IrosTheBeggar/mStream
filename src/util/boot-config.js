...
import { appRoot, isBunStandalone } from './esm-helpers.js';

// Per-OS user data home for the desktop profile. One root holds everything
// (config, db, caches, downloaded ffmpeg) — mStream's storage dirs are all
// config-pointable, so splitting across XDG config/data/cache dirs buys
// nothing but support burden. Casing follows each platform's convention.
export function userDataHome(platform = process.platform, env = process.env, homedir = osHomedir) {
  if (platform === 'win32') {
    // LOCALAPPDATA, not APPDATA: the db and art/waveform caches can reach
    // gigabytes and don't belong in a roaming profile.
...
//   2. Non-standalone runs ('server') — appRoot/save/conf/default.json.
//   3. --portable ('portable') — same path, by request.
...
export function resolveDefaultConfig({
  ...
  if (!standalone) { return { path: legacyPath, profile: 'server' }; }
