 ...
 * the music-discovery network) from GitHub Release assets on first use, with
 * SHA256 verification against a manifest COMMITTED to the repo — the sidecar
 * binaries themselves no longer live in git (each CI refresh of the 9-binary
 * set added ~172 MB of undeltifiable blobs to history forever; the committed
 * manifest is the few-hundred-byte replacement).
 *
 * Trust model (same shape as ffmpeg-bootstrap.js, but stricter — we pin exact
 * hashes, not a live upstream checksum file):
 *   - bin/p2p-sidecar/manifest.json (and manifest-musl.json) are committed by
 *     the build workflows, reviewed like any other tree change, and carry
 *     {file, sha256, size, url, sourceTree} per platform.
 *   - Assets live under an immutable naming scheme (tree hash + content hash
 *     in the filename) on a machine-managed GitHub release, so a manifest
 *     from ANY point in history keeps resolving to exactly the bytes it pins.
 *   - A download that does not hash to the manifest's sha256 is deleted and
 ...
 * the manifest's URL BASE (the sha256 pins still apply) for air-gapped
...
// The manifest entry for this platform, or null when none is published.
// A valid entry pins file name, size, and sha256; refuse malformed ones
// loudly rather than downloading something we can't verify.
export function manifestEntry({ manifestDir = defaultManifestDir(), key = sidecarKey() } = {}) {
  ...
  if (typeof entry.file !== 'string' || !/^[0-9a-f]{64}$/.test(entry.sha256 || '')
      || !Number.isInteger(entry.size) || entry.size <= 0 || typeof entry.url !== 'string') {
...
async function ensureInto({ manifestDir = defaultManifestDir(), installDir, key = sidecarKey(), probe = probeSidecar }) {
  ...
  // MSTREAM_SIDECAR_BASE swaps the URL base (mirror / test loopback); the
  // manifest's sha256 pin still gates what gets installed. Read lazily so
  // tests and mirrors don't depend on import order.
  const base = (process.env.MSTREAM_SIDECAR_BASE || '').replace(/\/+$/, '');
  const url = base ? `${base}/${entry.file}` : entry.url;

  winston.info(`[${FAMILY}] downloading ${entry.file} (${(entry.size / 1024 / 1024).toFixed(1)} MB) from ${base || 'GitHub release assets'}...`);
  try {
