...
test('a read-only app dir moves the server-profile default to the data root', () => {
  // #803's translocated-.app / root-owned-install fallback: dataRoot (passed
  // here as writableRoot) diverges from appRoot only when appRoot can't be
  // written, and the config default must follow it.
  const r = resolve({ standalone: false, writableRoot: '/fallback' });
  assert.deepEqual(r, { path: join('/fallback', 'save', 'conf', 'default.json'), profile: 'server' });
});
