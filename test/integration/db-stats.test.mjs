 ...
 *
 * These were restructured to drive FROM user_metadata via the V61 composite
 * indexes + an OR/COALESCE canonical-hash join, with the ordered ids rendered
 * through renderMetadataByIds (the batched trackQuery path search uses).
 ...
 *
 * Pattern mirrors test/integration/playlist-load.test.mjs: boot a real mStream
 * in public/no-users mode, seed the DB directly, hit the HTTP API.
 */
...
function seed(dbPath) {
  ...
  // >= (not ===) so the next migration doesn't break this test — V61 is when
  // the composite user_metadata stat indexes landed.
  assert.ok(db.prepare('PRAGMA user_version').get().user_version >= 61, 'V61+ applied at boot');
...
describe('homepage stats — rated / most-played / recently-played', () => {
  ...
  test('stats queries are served by the V61 composite + hash indexes', () => {
    // Not a behaviour check — locks in the perf mechanism so a future query
    // or index change that silently regresses to a tracks scan fails loudly
    // here. Mirrors userStatRows' SQL shape (src/api/db.js).
    const sql = `
      WITH c AS (SELECT track_hash, play_count AS stat
                   FROM user_metadata
      ...
      SELECT t.id
      FROM resolved t
      WHERE t.library_id IN (1)
      ORDER BY t.stat DESC
    `;
    const db = new DatabaseSync(path.join(tmpDir, 'db', 'mstream.db'), { readOnly: true });
    const steps = db.prepare(`EXPLAIN QUERY PLAN ${sql}`).all(1).map(s => s.detail).join(' | ');
    db.close();
    assert.match(steps, /USING (COVERING )?INDEX idx_user_metadata_user_playcount/,
      `plan does not seek the composite index: ${steps}`);
    assert.match(steps, /USING INDEX idx_tracks_audio_hash \(audio_hash=\?\)/,
      `audio-hash branch is not an index seek: ${steps}`);
    assert.match(steps, /USING INDEX idx_tracks_hash \(file_hash=\?\)/,
      `file-hash branch is not an index seek: ${steps}`);
    assert.doesNotMatch(steps, /USING INDEX idx_tracks_library/,
      `library filter leaked back into the join loop: ${steps}`);
  });
});
