/**
 * V73 migration: tracks.artist_display + credit roles in track_artists.
 *
 * ADD COLUMN only, no hook — the test pins the shape a V72 database ends up
...
function buildV72Fixture() {
  const db = new DatabaseSync(':memory:');
  db.exec('PRAGMA foreign_keys = ON');
  applyAllMigrations(db, { upToVersion: 72 });
  assert.equal(db.prepare('PRAGMA user_version').get().user_version, 72);
  db.exec(`
...
describe('V73 — artist_display + credit roles', () => {
  test('adds tracks.artist_display; legacy rows keep NULL; version + contract advance together', () => {
    const db = buildV72Fixture();
    applyAllMigrations(db, { fromVersion: 72 });
    assert.equal(db.prepare('PRAGMA user_version').get().user_version, SCHEMA_VERSION);
    assert.ok(SCHEMA_VERSION >= 73);
    assert.ok(SCANNER_SCHEMA_CONTRACT >= 73, 'the scanners write a new column → the contract moves with it');
    ...
    const entry = MIGRATIONS.find((m) => m.version === 73);
  ...
  test('track_artists takes the new roles next to a performer credit for the same artist', () => {
    ...
    // The V72 dirty triggers cover every role: the artists were flagged.
