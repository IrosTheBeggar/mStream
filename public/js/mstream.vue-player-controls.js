var VUEPLAYER = (function () {
  ...
  Vue.component('playlist-item', {
    template: '\
      <div class="noselect playlist-item" v-bind:class="{ playing: (this.index === positionCache.val), playError: (this.songError && this.songError === true) }" >\
        <span class="drag-handle"><img src="' + ROOT_PATH + '/public/img/drag-handle.svg"></span><span v-on:click="goToSong($event)" class="song-area">{{ comtext }}</span>\
        <div class="song-button-box">\
          ...
          <span v-on:click="createPopper($event)" class="songDropdown pop-c">\
            {{ratingNumber}}<img class="star-small pop-c" src="' + ROOT_PATH + '/public/img/star.svg">\
          </span>\
  ...
  new Vue({
    ...
    computed: {
      imgsrc: function () {
        return ROOT_PATH + "/public/img/" + (this.playerStats.playing ? 'pause' : 'play') + "-white.svg";
      },
      volumeSrc: function () {
        return ROOT_PATH + "/public/img/" + (this.playerStats.volume !== 0 ? 'volume' : 'volume-mute') + ".svg";
      },
      widthcss: function () {
      ...
      computed: {
        albumArtPath: function () {
          if (!this.meta['album-art']) {
            return ROOT_PATH + '/public/img/default.png';
          }
          return ROOT_PATH + '/album-art/' + this.meta['album-art'] + '?token=' + MSTREAMAPI.currentServer.token;
        }
      }
    });
  }
